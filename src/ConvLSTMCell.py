"""
This script contains the PyTorch implementation of the core ConvLSTM cell
of the agent model.
...
class ConvLSTMCell(nn.Module):
    ...
    Args:
        ...
        input_dim: The length of of side of input data. Data is
            presumed to have identical width and height.
        n_kernels: The number of kernels of Conv2d layers.
        dropout: The value of p parameter for dropout layers.
        batch_norm: Boolean value to indicate whether to use a batch
            normalization layer"""
    ...
    def __init__(self, input_channels, input_dim, n_kernels,
        ...
        if self.batch_norm:
            self.batch_norm_layer = nn.BatchNorm2d(
                num_features=self.input_channels)
    ...
    def forward(self, x):
        ...
        i_t = (sigmoid(self.W_xi(x) + self.W_hi(self.H) +
                       self.W_ci(self.C) + self.b_i))
        f_t = (sigmoid(self.W_xf(x) + self.W_hf(self.H) +
                       self.W_cf(self.C) + self.b_f))
        C_t = (f_t * self.C + i_t * tanh(self.W_xc(x) +
                                         self.W_hc(self.H) + self.b_c))
        C_t = self.C_drop(C_t)
        o_t = (sigmoid(self.W_xo(x) + self.W_ho(self.H) +
                       self.W_co(self.C) + self.b_o))
        H_t = o_t * tanh(C_t)
        H_t = self.H_drop(H_t)
